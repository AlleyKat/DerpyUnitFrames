...
W.Feed = function(self,var)
	if var.combatfeedback ~= true then return end
	local feed = self.level_:CreateFontString(nil, "OVERLAY")
	feed:SetPoint("CENTER")
	feed:SetFont(M["media"].fontn,min(max(var.h-16,14),24), "OUTLINE")
	feed:SetShadowOffset(1,-1)
	feed:SetJustifyH("CENTER")
	self.CombatFeedbackText = feed
end

if IsAddOnLoaded("oUF_CombatFeedback") then return end
local oUF = W.oUF
