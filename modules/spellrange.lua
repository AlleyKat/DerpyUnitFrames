...
local sp = V.common.spellrange 

local _update_fader = function(self,t)
	self.cur = self.cur + t*self.mod*self.speed
	if self.cur * self.mod > self.limit * self.mod then
		self.parent:SetAlpha(self.limit)
		self:Hide()
	else
		self.parent:SetAlpha(self.cur)
	end
end

-- we need new show in function
local check_alpha_and_run = function(self,newalpha)
	self.__fader:Hide()
	local x = self:GetAlpha()
	local c = x - newalpha
	if c > 0 then
		self.__fader.mod = -1
		self.__fader.speed = c >= 1 and 3.5 or c*2
	else
		self.__fader.mod = 1
		self.__fader.speed = -c >= 1 and 3.5 or -c*2
	end
	self.__fader.cur = x
	self.__fader.limit = newalpha
	self.__fader:Show()
end

W.SpellRange = function(self,var)
	if not var.rangealpha then return end
	if var.rangealpha == 100 then return end
	if not sp then return end
	self.__fader = CreateFrame("Frame",nil,self)
	self.__fader:SetScript("OnUpdate",_update_fader)
	self.__fader:Hide()
	self.__fader.parent = self
	self.check_alpha_and_run = check_alpha_and_run
	self.SpellRange = {
		["insideAlpha"] = 1,
		["outsideAlpha"] = var.rangealpha/100,}
end
...
local function UpdateRange ( self )
	...
	if ( ObjectRanges[ self ] ~= InRange ) then -- Range state changed
		...
		if ( SpellRange.Update ) then
			...
			self:check_alpha_and_run(SpellRange[ InRange and "insideAlpha" or "outsideAlpha" ]);
		end
	end
end
